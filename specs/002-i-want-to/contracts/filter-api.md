...
- Must find and cache references to: search input, direction dropdown, camera cards, no-results message
- Must populate direction dropdown with unique values from camera data
- Must attach event listeners to filter controls
 ...
 * Called by event handlers on search input and direction dropdown
 * 
 ...
 * Sets searchTerm to empty, clears all direction selections
...
/**
 * Populate direction dropdown with unique direction values
 * Extracts unique directions from camera data
 * 
 * @param {Camera[]} cameras - Array of all cameras
 * @returns {void}
 */
function populateDirectionDropdown(cameras)
```
...
/**
 * Handle direction dropdown change event
 * Updates filter state with new direction selection
 * 
 * @param {Event} event - Change event from direction dropdown
 * @returns {void}
 */
...
>
  <!-- Options populated dynamically by populateDirectionDropdown() -->
</select>
...
- All filter controls must be keyboard navigable
  ...
  - Direction dropdown: Native `<select multiple>` with arrow keys
  - Clear button: Standard `<button>` with Enter/Space
  
- ARIA labels required:
  - `aria-label` on search input and direction dropdown
